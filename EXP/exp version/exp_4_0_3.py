...
class Shvar(RememberInstanceCreationInfo):
    ...
    def check_leap_year(Year):
        ...
        if isinstance(Year, int) is False:raise TypeError("Year should be an integer.")
    ...
    def split_Filename(Filename):
        ...
        if isinstance(Filename, str) is False:raise Type("Filename should be a string.")
    ...
    def split_Content(Content):
        if isinstance(Content, str) is False:raise TypeError("Content should be a string.")
        ...
        if Content[0] == 'origin':
            ...
            for x in Xdata:
                ...
                if '+/-' in temp:
                    temp.split('+/-')
                    temp = ufloat(float(temp[0]), float(temp[1]))
                    Xdata[i] = temp
                else:
                    Xdata[i] = float(temp)
                i = i+1
