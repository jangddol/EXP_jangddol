...
class Shvar(RememberInstanceCreationInfo):
    ...
    ClassVarList = ['tp', 'origin', 'datalength', 'tbe', 'mean', 'vrc', 'stdv', 
                    'SE', 'uf', 'iftval', 'tval', 'length', 'dsc', 'scalar', 
                    'scalarlist', 'intercept', 'slope' ,'R2', 'names', 'grouptp', 'XYdata', 'varname']
    ...
    def setufdatalist(self, data, epdf='rec', ec=0, iftval=False, tval=0):
        ...
        This method produce following class variable.
            .tp         : ='ufdatalist'
            .origin     : Original data. =data
            .datalength : The list of length of data which was used to calculate each ufdata.
            .tbe        : Type-B uncertainty
        ...
        self.tp = 'ufdatalist'
        self.origin = data
        self.datalength = [len(data[i]) for i in range(N)]
    ...
    def change_one_to_two_digits(inputstr):
        ...
        if len(inputstr) == 1:
            ...
            return TyperError("inputstr should be a string of length 1 or 2.")
    ...
    def change_time_to_text(deltime = 0, timedeltype = 'past'):
        ...
        if isinstance(deltime, int) and deltime >= 0: # 매개변수 deltime은 양의 정수여야 한다.
            pass
        else:
            raise TypeError("deltime should be positive integer.")
        
        if timedeltype == 'past' or timedeltype == 'future': # timedeltype이 'past'면 현재시간에서 deltime만큼 뺀 시간을 구한다.
            pass # timedeltype이 'future'면 현재시간에서 deltime만큼 더한 시간을 구한다.
        else:
    ...
    def split_Filename(Filename):
        ...
        if isinstance(Filename, str):
            ...
            raise Type("Filename should be a string.")
    ...
    def split_Content(Content):
        ...
        if Content[0] == 'origin':
        ...
        elif Content[0] == 'mean' or Content[0] == 'vrc'or Content[0] == 'stdv'or Content[0] == 'SE' or Content[0] == 'tval':
            if Content[1][0] == '[':
                Content[1] = Content[1][1:-1]
                Content[1] = Content[1].split(',')
                i = 0
            ...
            if Content[1][0] == '[':
                ...
                while i < len(Content[1]):
                    ...
                    x[0] = float(x[0])
                    x[1] = float(x[1])
                    Content[1][i] = ufloat(x[0], x[1])
                    i = i+1
            else:
                Content[1] = Content[1].split('+/-')
                Content[1][0] = float(Content[1][0])
                Content[1][1] = float(Content[1][1])
                Content[1] = ufloat(Content[1][0], Content[1][1])
            ...
            Content[1] = float(Content[1])
        elif Content[0] == 'names':
            Content[1] = Content[1][1:-1]
            Content[1] = Content[1].split(',')
        elif Content[0] == 'grouptp':
            Content[1] = Content[1][1:-1]
            Content[1] = Content[1].split(',')
        elif Content[0] == 'XYdata': ########################## 개어려움
            temp = 0
            exec('temp =', Content[1])
            Content[1] = temp
        return Content
    ...
    def writeshvartext(self):
        # 텍스트 파일 기본 디렉토리
        BaseDirectory = '/content/drive/MyDrive/Coding/EXP/var/'
        if __name__ != '__main__':
            BaseDirectory = __file__.replace('exp.py', 'var/')
            if os.path.isdir(BaseDirectory.replace('var/', 'var')):
                pass
            else:
        ...
        if os.path.isdir(BaseDirectory + date):
            pass
        else:
        ...
        # 텍스트 내용을 정한다.
            ...
            # 마지막에 작성 시간을 추가한다.
        backdata = [None for i in range(20)]
        backdata[0] = self.tp
        backdata[1] = self.origin
        backdata[2] = self.length
        backdata[3] = self.tbe
        ...
        backdata[15] = self.intercept
        backdata[16] = self.slope
        backdata[17] = self.R2
        ...
        for x in backdata:
            if x is None:
                ...
                content.append(Shvar.ClassVarList[i] + ', ' + str(x))
            i = i+1
    ...
    def readshvartext(varname, Date='today', Time='latest', imp=False):
        ...
        if Date == 'today': # 오늘일 경우의 디렉토리 이름
            ...
            if isinstance(Date, int):
                if len(str(Date)) <= 8: # 날짜는 8자리보다 작아야 한다.
                    ...
                    if Month >= 1 and Month <= 12: # 월의 입력을 검사한다.
                        pass
                    else:
        ...
        # 지정 날짜의 디렉토리를 불러온다.
            # 디렉토리 안에서 파일 이름들을 리스트로 불러온다. ㅎㅇ
            # 파일 이름들을 varname과 시간으로 분류한다. ㅎㅇ
            # varname이 있는지 검사하고 없으면 없다고 알리고 끈다. ㅎㅇ
            # varname이 있으면 지정된 시간을 불러온다. ㅎㅇ
            # 만약 latest면, 지정 varname 파일들을 시간순으로 나열한 후 마지막 파일을 불러온다.
            # 만약 지정된 시간이면, 지정 varname 파일들 중에서 해당 시간의 파일이 있는지 검사한다.
            # 없으면 없다고 알리고 끈다.
            # 있으면 해당 파일의 내용을 리스트로 불러온다.
            # 쉼표를 기준으로 각 줄의 내용을 찢어낸다.
            # 앞의 내용에 맞춰서 class variable에 넣어준다.
        ...
        if Time == 'latest': # 입력 시간을 '최신'으로 했을 경우
            ...
            if isinstance(Time, int) and len(str(Time)) <= 4: # 입력은 시간분을 연달아 입력한 4자리 정수여야 한다.
                ...
                Hour = int(strTime[:2])
                TitleMinute = int(int(strTime[2:])/30)*30
                if Hour >=0 and Hour < 24: # 시간값 검사
                    pass
                else:
                    raise ValueError("Hour must be in 0 ~ 23")
                
                if TitleMinute ==0 or TitleMinute == 30: # 분값 검사
                    pass
                else:
                ...
                while i < len(HourList): # 입력한 시간에 해당하는 값이 HourList에 있는지 검사
                    if HourList[i] == Hour:
                        AvailableMinuteList.append(Minute)
                    else:
                        pass
                    i = i+1
                ...
                if len(AvailableMinuteList) == 0: # 입력한 시간에 해당하는 값이 없으면 없다고 알리고 종료
                    ...
                    if TitleMinute in AvailableMinuteList: # 입력한 분에 해당하는 파일이 있는지 검사
                        pass
                    else: # 없으면 없다고 알리고 종료
                        print("There is no file about " + varname + '_' + Titletime + '.txt')
                        return None
        ...
        if imp is True:
            ...
            if '=' in context:
                ...
                if eqnum == 1:
                    context = context.replace(' ', '')
                    tempvarname = context.split('=')[0]
                else:
                    if context.count('readshvartext') == 1:
                        context = context.split('readshvartext')[0]
                        context = context.split(';')[-1]
                        context = context.split('=')[0]
                        tempvarname = context.replace(' ', '')
                    else:
