...
class Shvar(RememberInstanceCreationInfo):
    ...
    def change_time_to_text(deltime = 0, timedeltype = 'past'):
        ...
        if deltime != 0:
            ...
            while i < delzerosnum:
                delzeros = delzeros + '0'
                i = i+1
            deltime_text = delzeros + str(deltime)
            delDay = deltime_text[:2]
        ...
        TitleMinute = str(30 * int(int(Minute) / 30))
    ...
    def readshvartext(varname, Date='today', Time='latest', imp=False):
        ...
        if os.path.isdir(BaseDirectory + Directorytitle):
            FileList = os.listdir(BaseDirectory + Directorytitle)
            print(FileList)
            varnameList = []
            HourList = []
            MinuteList = []
        ...
        if Time == 'latest':
            ...
            MinuteList = [int(x) for x in MinuteList]
            AvaillatestMinuteList = []
            i = 0
            while i < len(HourList):
                if HourList[i] == Max(HourList):
                    AvaillatestMinuteList.append(MinuteList[i])
                else:
                    pass
                i = i+1
            Hour = str(Max(HourList))
            if len(Hour) == 1:
                Hour = '0' + Hour
            TitleMinute = str(Max(AvaillatestMinuteList))
            if len(TitleMinute) == 1:
                TitleMinute = '0' + TitleMinute
            Titletime = Hour + ':' + TitleMinute + ':00'
        else:
            if isinstance(Time, int) and len(str(Time)) <= 4:
                zerosnum = 4 - len(str(Time))
                zeros = ''
                i = 0
                while i < zerosnum:
                    zeros = zeros + '0'
                ...
                if len(AvailableMinuteList) == 0:
                    print("There is no file about " + varname + '_' + Titletime + '.txt')
                    return None
                else:
                    if TitleMinute in AvailableMinuteList:
                        pass
                    else:
                        print("There is no file about " + varname + '_' + Titletime + '.txt')
                        return None
